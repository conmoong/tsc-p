...
|---|---|---|
| `main` | Pristine mirror of upstream `main` | Never patch it. Fast-forward only. |
| `dev` | Default branch: pinned upstream stable release + the tsc-p patch | All real work happens here. |
| `nightly` | Nightly canary output (latest upstream `main` + patch merged) | **Disposable.** Force-pushed by automation nightly. Never commit to it, never branch feature work from it, never treat its state as durable. |

Upstream is synced by **merging** stable release tags into `dev`, never
by rebasing — this is deliberate so the `.gitattributes` `merge=ours`
entries keep working and history stays honest. See
