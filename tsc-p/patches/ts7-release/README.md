...
| Patch | Why |
|---|---|
| `0001-inherit-plugins-across-extends.patch` | The `compilerOptions.plugins`/`extends` fix. On `main` it sits next to the `contentMappers` handling, which does not exist on `ts7-release`, so the `main` diff cannot apply. The fix itself is permanent — it was reported upstream ([microsoft/TypeScript#63975](https://github.com/microsoft/TypeScript/issues/63975)) and declined ("We don't support plugins. We just ignore that entirely, let alone do any extending"), so it will not arrive from upstream. This *override* can go away once the lane picks up `contentMappers` and the single `main` diff applies to both lanes again. |
